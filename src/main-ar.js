...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Project Carousels - Modified for RTL
  ...
  carousels.forEach((carousel) => {
    ...
    // Function to show next slide - modified for RTL
    const nextSlide = (e) => {
      if (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    // Function to show previous slide - modified for RTL
    const prevSlide = (e) => {
      if (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    if (slides.length > 0) {
      ...
      // Set up button event listeners for RTL layout
      if (prevBtn && nextBtn) {
        if (isRTL) {
      ...
      // Add keyboard navigation - Modified for RTL
